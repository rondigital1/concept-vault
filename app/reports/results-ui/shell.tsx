...
import type { ResultsNavItem, ResultsNavKey } from './types';
...
const SIDE_NAV_ITEMS: ResultsNavItem[] = [
      ...
      <div className="mx-auto flex h-full max-w-[1560px] items-center justify-between px-6 lg:px-10">
        <Link href="/" className="flex items-center gap-3 transition-opacity hover:opacity-85">
          ...
          <div className="leading-none">
            <div className="text-[1.2rem] font-black tracking-normal text-white">Concept Vault</div>
          ...
          {TOP_NAV_ITEMS.map((item) => (
            <Link
              ...
              className={joinClassNames(
                'relative text-[1.1rem] font-medium tracking-normal transition-colors',
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/agents"
            className="rounded-full p-2 text-white/90 transition hover:bg-white/5"
          ...
          <Link
            href="/today"
            className="relative rounded-full p-2 text-white/90 transition hover:bg-white/5"
            aria-label="Open research queue"
          >
...
function MobileNav({ activeNav }: { activeNav: ResultsNavKey }) {
  return (
      ...
      <div className="flex flex-wrap gap-2">
        {SIDE_NAV_ITEMS.map((item) => (
          <Link
            ...
            href={item.href}
            className={joinClassNames(
              'inline-flex min-h-10 items-center gap-2 whitespace-nowrap rounded-full px-3.5 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.16em]',
    ...
    <>
      <aside className="fixed left-0 top-16 hidden h-[calc(100vh-4rem)] w-64 bg-[#151515] px-5 py-6 lg:flex lg:flex-col">
        <Link href="/reports" className="px-4 transition-opacity hover:opacity-85">
          <p className="text-[0.76rem] font-bold uppercase tracking-[0.08em] text-white">Reports</p>
          <p className="mt-1 text-[0.76rem] uppercase tracking-[0.08em] text-[#747070]">Approved dossiers</p>
        </Link>
...
export function ResultsRouteShell({
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#131313] text-[#ece9e8]">
      ...
      <main className="relative px-4 pb-16 pt-48 sm:px-6 lg:ml-64 lg:px-10 lg:pt-24">
        {children}
        ...
        {showReadyPulse ? (
          <div className="pointer-events-none fixed bottom-8 right-8 hidden flex-col items-center gap-4 xl:flex">
            ...
            <div className="text-[0.58rem] font-bold uppercase tracking-[0.34em] text-[#857e7e]">Archive ready</div>
          </div>
        ) : null}
      </main>
    </div>
