...
export default function ChatPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const responseContent = await chatAction(userMessage.content, messages);
      setIsTyping(false);
      ...
      const mockResponse: Message = {
        id: Date.now().toString(),
        role: 'assistant',
        content: `I received: "${userMessage.content}". This is a mock response as the backend is not connected yet.`,
        timestamp: new Date(),
      };
  ...
  const handleSaveWithTitle = async (title: string) => {
    try {
      const result = await saveToLibraryAction(textToSave, title);
      ...
      if (result.success) {
        ...
        console.log('Saved to library:', result.documentId, 'with title:', result.title, 'created:', result.created);
      } else {
