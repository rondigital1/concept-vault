...
export function buildTopicDraft(selectedTopic: AgentsView['selectedTopic']): TopicWorkflowDraft | null {
  if (!selectedTopic) {
    return null;
  }
...
export function updateTopicDraftField(
  current: TopicWorkflowDraft | null,
  field: string,
  value: string | number | boolean,
): TopicWorkflowDraft | null {
  if (!current) {
    return current;
  }
